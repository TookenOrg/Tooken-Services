...
func (h *Handler) AddClaimToIdentity(gCtx *gin.Context) {
	...
	txHash, err := h.blockchainSvc.AddClaimToIdentity(gCtx.Request.Context(), req)
	if err != nil {
	...
	gCtx.JSON(http.StatusOK, server.AddClaimResponse{
		...
		Data:    &server.TxHashName{TransactionHash: txHash.Hash().Hex(), OperationName: "AddClaimToIdentity"},
	})
}
