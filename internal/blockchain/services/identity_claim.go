...
func addClaim(ctx context.Context, identityInstance *contracts.Identity, addClaimRequest server.AddClaimRequest, signature models.SignatureResult) (tx *types.Transaction, err error) {
	...
	err = database.InsertEthTransaction(ctx, deployedTxDetails.Tx.Hash().Hex(), "ADD_CLAIMS_INDENTITY", deployedTxDetails.ToAddressHex(), deployedTxDetails.BlockNumber.Int64(), big.Int{})
	if err != nil {
		return
	}
